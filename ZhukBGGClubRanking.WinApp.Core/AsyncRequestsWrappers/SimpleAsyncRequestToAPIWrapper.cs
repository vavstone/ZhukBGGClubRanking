...
{
    public class SimpleAsyncRequestToAPIWrapper
    {
        public EventHandler WorkCompleted { get; set; }
        private BackgroundWorker bw = new BackgroundWorker();
        public Func<string, string, string, string, Task<HttpResponseMessage>> MethodToExecute { get; set; }

        public SimpleAsyncRequestToAPIWrapper()
        {
        ...
        public void DoWork(HostingSettings hostingSettings)
        {
            ...
            if (!bw.IsBusy) bw.RunWorkerAsync(prm);
        }
    }
}
